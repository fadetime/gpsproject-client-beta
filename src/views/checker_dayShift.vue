<template>
    <div id="checkerpage">
                ...
                <div class="checkbox-box">
                    ...
                    <div class="checkbox-box-body">
                        ...
                        <div class="checkbox-box-body-top">
                            ...
                            <div class="checker_km_item">
                                <div class="checker_km_item_left">
                                    <span v-if="lang === 'ch'">已行驶的公里数</span>
                                    <span v-else>Mileage drive(Km)</span>
                                </div>
                                <div class="checker_km_item_right">
                                    <input type="text" v-model="tempData.kilometer">
                                </div>
                            </div>
                            <div class="checker_km_item" style="margin-top:4px;">
                                <div class="checker_km_item_left">
                                    <span v-if="lang === 'ch'">最后更换机油公里</span>
                                    <span v-else>Last engineOil(Km)</span>
                                </div>
                                <div class="checker_km_item_right">
                                    <div class="checker_km_item_right_div" @click="changeEngineOilMethod()">
                                        <span>{{lastOilKelometer}}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="checkbox-box-body-top">
                            ...
                            <div>
                                <input type="text" style="border: 1px solid #eee;height: 30px;text-align:center;width: 80%;border-radius: 5px;" v-model="tempData.note">
                            </div>
                        </div>
                    </div>
                    <div class="checkbox-box-bottom">
        ...
        <!-- change engine box start -->
        <transition name="tips-classes-transition"
                    enter-active-class="animated fadeIn"
                    leave-active-class="animated fadeOut">
            <div v-if="isShowChangeOilBox" style="z-index:101" class="checkbox-back"></div>
        </transition>
        <transition name="tips-classes-transition"
                    enter-active-class="animated bounceInDown"
                    leave-active-class="animated bounceOutUp">
            <div v-if="isShowChangeOilBox" class="checkbox-front" style="z-index:102">
                <div class="checkbox-box">
                    <div class="confirmbox-box-title">
                        <span v-if="lang === 'ch'">修改车辆机油信息</span>
                        <span v-else>Change engine oil info</span>
                    </div>
                    <div class="checker_change_oil_body">
                        <div class="checker_change_oil_body_item">
                            <div class="checker_change_oil_body_item_left">
                                <span v-if="lang === 'ch'">原有数据(Km)</span>
                                <span v-else>Old info(Km)</span>
                            </div>
                            <div class="checker_change_oil_body_item_right">
                                <div>
                                    <span>{{lastOilKelometer}}</span>
                                </div>
                            </div>
                        ...
                        <div class="checker_change_oil_body_item">
                            <div class="checker_change_oil_body_item_left">
                                <span v-if="lang === 'ch'">修改数据(Km)</span>
                                <span v-else>New info(Km)</span>
                            </div>
                            <div class="checker_change_oil_body_item_right">
                                <input type="number" v-model="newOilNum">
                            </div>
                        </div>
                        
                    </div>
                    <div class="confirmbox-box-bottom">
                        <div class="confirmbox-box-bottom-button" @click="isShowChangeOilBox = false">
                            <span v-if="lang === 'ch'">取消</span>
                            <span v-else>Cancel</span>
                        </div>
                        <div class="confirmbox-box-bottom-button" style="margin-left:10px" @click="confirmChangeOilNumber">
                            <span v-if="lang === 'ch'">确定</span>
                            <span v-else>Confirm</span>
                        </div>
                    </div>
...
export default {
    ...
    data() {
        return {
            ...
            firstPageImage: null,
            lastOilKelometer: null,
            childDialogMsg:null,
            isShowChildValue:false,
            isShowChangeOilBox:false,
            newOilNum:null,
            car_id:null
        };
    },
    ...
    methods: {
        confirmChangeOilNumber(){
            if(this.newOilNum){
                if(this.newOilNum < this.lastOilKelometer){
                    if (this.lang === "ch") {
                        this.errorInfo = "公里数小于原有数值";
                    } else {
                        this.errorInfo = "The num less than original value";
                    }
                    this.showError = true;
                    setTimeout(() => {
                    ...
                    axios
                        ...
                        .then(doc => {
                            if(doc.data.code === 0){
                                this.isShowChangeOilBox = false
                                this.lastOilKelometer = this.newOilNum
                            }else{
                                if (this.lang === "ch") {
                                    this.errorInfo = "更新失败";
                                } else {
        ...
        secondBottonValue(secondBottonValue) {
            if(!secondBottonValue){
                ...
                this.isShowChildValue = false
                this.isShowChangeOilBox = true
            }
        },

        changeEngineOilMethod(){
            this.isShowChildValue = true
            if(this.lang === 'ch'){
                this.childDialogMsg = '确定修改车辆机油信息?'
            }else{
                this.childDialogMsg = 'Do you confirm change engine oil info?'
            }
...
.checker_km_item_left{
    margin-right: 8px;
    width: 140px;
    text-align: right;
}

.checker_km_item_right input{
    height: 30px;
...
.checker_change_oil_body{
    padding: 8px 12px;
}

.checker_change_oil_body_item{
    display: -webkit-flex;
    display: flex;
    height: 30px;
    line-height: 30px;
...
.checker_change_oil_body_item_left{
    width: 90px;
    text-align: right;
}

.checker_change_oil_body_item_right div{
    width: 120px;
    margin-left: 8px;
    border: 1px solid #eee;
    height: 30px;
...
.checker_change_oil_body_item_right input{
    width: 120px;
    margin-left: 8px;
    border: 1px solid #eee;
    height: 30px;
